...
class Admin extends Controller
{
		...
		if(isset($_POST['post_verify'])){
		 if ($input->exists()) {
			...
			if (!$validation->fails()) {
				...
				$update = $settingsModel->verify_email($input->get('verify_email'));
					
				if ($update == 1) {
					$_SESSION['message'][] = ['success', $this->lang['details_updated']];
					redirect('admin/settings/verify_email');
				} else {
					$_SESSION['message'][] = ['warning', $this->lang['no_changes_made']];
					redirect('admin/settings/verify_email');
				}
    ...
    public function roles()
    {
       ...
       if(isset($this->url[2]) && $this->url[2] == 'list') {
           
        $data['roles'] = $staffModel->roles_list();

        return ['content' => $this->view->render($data, 'admin/roles_list')];
        
       }elseif(isset($this->url[2]) && $this->url[2] == 'add') {
                ...
                if(isset($_POST['add_roles'])){
                 if ($input->exists()) {
                    ...
                    if (!$validation->fails()) {		
                            ...
                            $Insert = $staffModel->roles_add($input->get('name'), 
                                                       $input->get('staff'), 
                                                       $input->get('projects'), 
                                                       $input->get('categories'), 
                                                       $input->get('skills'), 
                                                       $input->get('users'), 
                                                       $input->get('portfolios'), 
                                                       $input->get('verification_requests'), 
                                                       $input->get('settings'), 
                                                       $input->get('pages'), 
                                                       $input->get('revenue_settings'), 
                                                       $input->get('revenues'), 
                                                       $input->get('escrow_payments'), 
                                                       $input->get('funds'), 
                                                       $input->get('withdrawals'), 
                                                       $input->get('disputes'), 
                                                       $input->get('boxplace')); 
                ...
                return ['content' => $this->view->render($data, 'admin/roles_add')];

        // Edit
        }elseif(isset($this->url[2]) && $this->url[2] == 'edit') {			
			
            $has = $staffModel->has_role($this->url[3]);

            // If the currency requested exists
            if($has === true) {
				
				/* User data */
                $data["role"] = $staffModel->get_role($this->url[3]);
				...
				if(isset($_POST['edit_roles'])){
				 if ($input->exists()) {
					...
					if (!$validation->fails()) {
                            ...
                            $update = $staffModel->roles_update($input->get('name'), 
                                                       $input->get('staff'), 
                                                       $input->get('projects'), 
                                                       $input->get('categories'), 
                                                       $input->get('skills'), 
						...
						if ($update == 1) {
							$_SESSION['message'][] = ['success', $this->lang['details_updated']];
							redirect('admin/roles/edit/'. $input->get('id'));
						} else {
							$_SESSION['message'][] = ['warning', $this->lang['no_changes_made']];
							redirect('admin/roles/edit/'. $input->get('id'));
						}
                ...
                return ['content' => $this->view->render($data, 'admin/roles_edit')];
    ...
    public function staff()
    {
       ...
       if(isset($this->url[2]) && $this->url[2] == 'list') {
        ...
        $data['staffs'] = $staffModel->list();

        return ['content' => $this->view->render($data, 'admin/staff_list')];
                ...
                if(isset($_POST['add_staff'])){
                 if ($input->exists()) {
                    ...
                    if (!$validation->fails()) {
                            ...
                            /* Unique ID */	
                            $adminid = $this->uniqueid();				

                            $Insert = $staffModel->add($adminid, $input->get('name'), $input->get('email'), $password, $input->get('role')); 
                ...
                return ['content' => $this->view->render($data, 'admin/staff_add')];

        // Edit
        }elseif(isset($this->url[2]) && $this->url[2] == 'edit') {			
			
            $has = $staffModel->has($this->url[4]);
            ...
            if($has === true) {
				...
				/* User data */
                $data["admin"] = $staffModel->get($this->url[4]);
				...
				if(isset($_POST['edit_details'])){
				 if ($input->exists()) {
					...
					if (!$validation->fails()) {
						...
						$update = $staffModel->update_staff($input->get('name'), $input->get('email'), $input->get('role'), $input->get('adminid'));
							
						if ($update == 1) {
							$_SESSION['message'][] = ['success', $this->lang['details_updated']];
							redirect('admin/staff/edit/details/'. $input->get('adminid'));
						} else {
							$_SESSION['message'][] = ['warning', $this->lang['no_changes_made']];
							redirect('admin/staff/edit/details/'. $input->get('adminid'));
						}
					  ...
					  if($_FILES['photoimg']['error'] == 0 && in_array($fileFormat, $valid_formats))
					   {
						 $fileName = $this->rando().'.'.$fileFormat;
						 // If the file can't be written on the disk (will return 0)
						 $path = sprintf('%s/../../%s/%s/admin/staff/', __DIR__, PUBLIC_PATH, UPLOADS_PATH);
						 ...
						 if(move_uploaded_file($_FILES['photoimg']['tmp_name'], $path.$fileName))
						  {
							...
							$update = $staffModel->changeImage($fileName, $input->get('adminid'));
							
							if ($update == 1) {
								$_SESSION['message'][] = ['success', $this->lang['details_updated']];
							    redirect('admin/staff/edit/image/'. $input->get('adminid'));
							} else {
								$_SESSION['message'][] = ['warning', $this->lang['no_changes_made']];
							    redirect('admin/staff/edit/image/'. $input->get('adminid'));
							}		  
				 ...
				 if ($input->exists()) {
					...
					if (!$validation->fails()) {
						...
						if (password_verify($input->get('password_current'), $input->get('db_password'))) {
							...
							$update = $staffModel->password($password, $input->get('adminid'));
								
							if ($update == 1) {
								$_SESSION['message'][] = ['success', $this->lang['details_updated']];
							    redirect('admin/staff/edit/password/'. $input->get('adminid'));
							} else {
								$_SESSION['message'][] = ['warning', $this->lang['no_changes_made']];
							    redirect('admin/staff/edit/password/'. $input->get('adminid'));
							}
                ...
                return ['content' => $this->view->render($data, 'admin/staff_edit')];
